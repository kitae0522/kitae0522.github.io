...
```mermaid
flowchart TB
  Lab["LG Gram lab\n16GB RAM · 256GB SSD"] --> Runner["GitHub runner 4개"]
...
## 1순위는 계속 GitHub self-hosted runner다

이 서버를 만든 직접적인 이유입니다. private repository의 테스트, lint와 Docker image build를 처리합니다.

지금은 bot repository가 세 슬롯, CI image repository가 한 슬롯을 사용합니다. 앞으로 repository가 늘어나면 네 슬롯을 다시 나누거나 두 번째 서버를 추가해야 합니다. runner가 자동으로 무한 확장되지는 않습니다.

```mermaid
flowchart LR
  Repos["내 private repositories"] -->|"작은 test · lint"| Lab["lab-small\nrunner 4개"]
  Repos -->|"큰 build · 신뢰할 수 없는 PR"| Hosted["GitHub-hosted runner"]
```
...
flowchart LR
  ...
  Test --> Lab["lab에서 실행"]
  Lab --> Access["Tailscale + SSH tunnel"]
```

처음에는 저만 쓰도록 `127.0.0.1`에 바인딩합니다. 다른 사람에게 공개해야 할 이유가 생겼을 때 도메인, TLS와 인증을 추가합니다. 토이 프로젝트라고 처음부터 인터넷 전체에 포트를 열 필요는 없습니다.
