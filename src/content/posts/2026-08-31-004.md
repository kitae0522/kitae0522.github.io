...
## 5. 집 밖에서는 SSH tunnel로 확인한다

서비스를 `127.0.0.1:8080`에 바인딩했기 때문에 외부에서는 직접 열리지 않습니다. Mac에서 다음 tunnel을 만듭니다.

```bash
ssh -N -L 8080:127.0.0.1:8080 ted@lab
```

그 뒤 Mac 브라우저에서 `http://127.0.0.1:8080`을 엽니다.

```mermaid
flowchart LR
  Browser["Mac 브라우저\n127.0.0.1:8080"] --> SSH["Tailscale 안의 SSH tunnel"]
  SSH --> App["lab의 hello-home\n127.0.0.1:8080"]
  Public["외부 인터넷"] -->|"직접 접근 불가"| Closed["공개 포트 없음"]
```

다른 사람도 사용해야 하는 토이 프로젝트라면 이 단계가 달라집니다. 도메인, TLS, reverse proxy나 공개 tunnel, 인증과 rate limit을 먼저 설계해야 합니다. 지금의 `lab`은 아직 공개 웹서비스 경로를 제공하지 않습니다.
